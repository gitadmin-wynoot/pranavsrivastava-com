      ...
      {!mode ? <>
        ...
        {panelOpen && <aside className={s.learning} aria-label={mode==="lab"?"Architecture controls":"Camp field notes"}>
          ...
          <div className={s.panelScroll}>
            {mode==="live" && <LiveSidePanel stageIndex={stageIndex} elapsedAt={history.at(-1)?.at ?? 0} paused={paused} incidentTitle={incident?.title} feed={session.liveFeed} earned={session.earned} agentCount={architecture.agentCount} metrics={metrics} />}

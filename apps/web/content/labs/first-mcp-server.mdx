...
MCP is a standard way to give an AI model access to tools and data through a small server it can talk to — instead of hard-coding integrations into the model.
</Callout>

<Diagram title="What you are building" caption="A client asks the model; the model calls your tool through MCP">
{`
  AI client  ──asks──▶  Model  ──tool call──▶  Your MCP server
      ▲                                              │
      └──────────────── answer ◀────────────────────┘
`}
</Diagram>

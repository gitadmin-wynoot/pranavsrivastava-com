...
<Diagram title="Things become points in space" caption="Once described as numbers, similar things land near each other">

```
        income
          |
     52k  •  Anna [34, 178, 52k]
          |   • Ben [36, 175, 50k]   <- near Anna, so "similar"
          |
     20k  |              • Cara [22, 165, 20k]
          +---------------------------- age
             20        34   36
...
<Diagram title="Learning is walking downhill" caption="Each step reduces the error a little, until it can't go lower">

```
  error
   high  \
          \•  start (model is wrong)
           \
            \•  step... feel the slope... step down
             \
              \•
               \___•___ bottom = model is as right as it can get
                       (lowest error)
```

...
Forget the scary word for a second. Imagine a giant **map**, and on it, every word gets its own little house. Here is the trick: words that *mean* similar things get houses **near each other** — all the animals in one neighbourhood, all the foods in another, all the things-that-go somewhere else. Tap a word and watch it find its closest friends:
...
<Figure name="vectors-space" caption="An embedding turns text into a point. Similar meanings become nearby points — so 'how much leave do I get' lands near 'vacation policy', with no shared words." />

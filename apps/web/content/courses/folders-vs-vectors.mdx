...
A **vector** sounds mathematical; the idea is not. An <KeyTerm definition="A list of numbers representing a piece of text's meaning, arranged so that similar meanings sit close together.">embedding</KeyTerm> turns a piece of text into a long list of numbers — a *point in space* — placed so that texts with **similar meaning land near each other**, even if they share no words.
...
That is the superpower: you can search by **meaning**, not spelling. Ask "how much time off do I get" and it finds the passage about "annual leave." Try it — type a question and watch which passages get retrieved:

<RetrievalPlayground />

A <KeyTerm definition="A database that stores embeddings and can quickly find the vectors nearest to a query vector.">vector database</KeyTerm> is just a store of these number-points plus fast "find the nearest points" search. Feed it thousands or millions of chunks, and it retrieves the handful closest in meaning to any question. Wrap that in a model that answers *from* those chunks, and you have RAG — covered end to end in [RAG in Production](/courses/production-rag).

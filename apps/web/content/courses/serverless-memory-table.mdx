...
Picture a colleague with a strange condition: total amnesia every time they finish a task. Ask them a question, they answer, then they forget you ever spoke. That is fine for quick questions. But hand them something that needs a pause — "check with the manager first, then come back to this" — and they simply *can't*. The moment they finish speaking to you, the whole thread of what they were doing is gone. There is no "them" to come back to it.

Now give that colleague one thing: a notebook. Before they forget, they scribble down exactly what they were doing and how far they got. Next time you need them, a fresh version of them — same skills, zero memory of before — opens the notebook, reads where things stood, and carries on as if no time had passed at all. The notebook is what makes the amnesia survivable. It turns a person who resets every few minutes into someone who can run a project across days.

That colleague is **AWS Lambda**. It really does forget everything the instant it finishes running, and it is capped at fifteen minutes per turn, no exceptions. And the notebook is what this course builds: a durable memory table in **DynamoDB** that lets a **LangGraph** agent — one that sometimes has to stop and ask a human "are you sure?" before booking a slot, moving money, or changing a record — pause for as long as it needs to, and then pick up exactly where it left off, on a completely different, memory-less Lambda run. By the end you will understand precisely why that notebook is necessary, and you will have a complete, production-ready, multi-tenant implementation of it.

<Callout type="info" title="What this really teaches">
The specific artefact is a checkpointer. The transferable idea is bigger — and it is the notebook trick, generalised: how to give a stateless, time-boxed function a *memory*, so it can act like a reliable, long-running process instead of a goldfish. That pattern shows up far beyond agents.
</Callout>
